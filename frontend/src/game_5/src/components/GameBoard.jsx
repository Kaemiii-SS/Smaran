...
function GameBoard({
  ...
  return <main className="relative flex items-center justify-center m-auto rounded-md w-full py-4 h-full">
      {
    ...
    className={cn(
      ...
      (!gameStarted || isPaused) && "opacity-30 pointer-events-none"
      // Dim the board when not started or paused
    )}
    style={{
  ...
  />
    ...
    /* Overlay when game is paused */
      ...
      {isPaused && gameStarted && <div className="absolute inset-0 z-10 flex flex-col items-center justify-center pointer-events-auto text-center w-full h-full">
          <motion.div
