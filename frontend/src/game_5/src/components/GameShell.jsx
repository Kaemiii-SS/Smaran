...
import { ArrowLeft, Music, Layers, Play, Pause } from "lucide-react";
...
function GameShell({
  ...
  return <div
    ...
    onClick={!instructionsDismissed ? handleDismissInstructions : void 0}
  >
      {
    /* Background Video */
  }
  ...
  />
    ...
    /* Instructions Overlay */
      ...
      <AnimatePresence>
        {!instructionsDismissed && <motion.div
    key="instructions"
  ...
  >
              ...
              <div className="mt-2 px-6 py-2 bg-[#16a34a] rounded-xl border border-[#16a34a]/20 shadow-md">
                <p className="text-xs text-white font-bold tracking-wide animate-pulse">
  ...
  >
    ...
    className={cn(
      ...
      isPaused ? "bg-amber-500 text-white hover:bg-amber-600" : "bg-white/50 text-charcoal hover:bg-white"
    )}
  >
    ...
    /* ── Absolute Floating Score Card (Right) ── */
      ...
      <div className="absolute right-6 top-24 z-20 w-48 hidden md:block">
        <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl border-4 border-white/60 p-5 font-bold text-charcoal">
          ...
          {streak > 1 && <div className="mt-3 text-center border-t border-black/5 pt-2">
