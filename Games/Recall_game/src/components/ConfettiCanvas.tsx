...
function makeParticle(x: number, y: number, { startVelocity, spread, gravity }: ParticleOpts): Particle {
  const angleDeg = 90 + (Math.random() - 0.5) * spread; // 90deg = straight up
  const angle = (angleDeg * Math.PI) / 180;
  const speed = startVelocity * (0.6 + Math.random() * 0.4) * 10; // px/s
...
export interface ConfettiCanvasHandle {
  burst: (x: number, y: number, opts?: Partial<ConfettiBurstOptions>) => void;
}
...
export const ConfettiCanvas = forwardRef<ConfettiCanvasHandle, {}>(function ConfettiCanvas(_props, ref) {
  ...
  useImperativeHandle(ref, () => ({
    burst(x: number, y: number, opts: Partial<ConfettiBurstOptions> = {}) {
      const { 
        particleCount = 100, 
        startVelocity = 30, 
        spread = 360, 
