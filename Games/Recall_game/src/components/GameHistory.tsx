...
export function GameHistory({ history }: GameHistoryProps) {
  ...
  const otherScores = history.filter(score => !score.isHighScore)
    .sort((a, b) => a.time - b.time);

  return (
    <Card className="shadow-none hover:shadow-lg rounded-lg w-full">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          {highScore && (
            <div className="flex items-center gap-1.5 md:gap-3 text-base md:text-xl text-muted-foreground">
      ...
      <CardContent className="space-y-3">
        {history.length === 0 ? (
          ...
          <div className="space-y-2">
            {/* High Score */}
            {highScore && (
              <div className="bg-gradient-to-r from-yellow-500/10 to-yellow-500/5 p-3 rounded-md border border-yellow-500/30">
