...
import { GAME_NODES, RhythmNode } from '@/lib/nodes';
...
export function useRhythmGame() {
  ...
  const [sequence, setSequence] = useState<RhythmNode[]>([]);
  ...
  const [errorPad, setErrorPad] = useState<string | null>(null);
  ...
  const playSequence = async (seq: RhythmNode[], speed: number) => {
  ...
  const handlePadClick = async (node: RhythmNode) => {
  ...
  return {
    difficulty,
    setDifficulty: (d: Difficulty) => { setDifficulty(d); resetGame(); },
    level,
    streak,
    isPlaying,
    isPlayerTurn,
